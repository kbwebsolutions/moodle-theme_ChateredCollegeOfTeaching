...
class mediaresource_controller extends controller_abstract {
    ...
    private function get_media_html($resource, $context, \cm_info $cm) {
        ...
        if (count($files) < 1) {
            ...
            // Modal title.
            $content = "<h5 class='charteredcollege-lightbox-title'>".format_string($resource->name)."</h5>";

            // Grid me up.
            if (!empty($resource->intro)) {
                $lightboxgrid = "<div class='col-sm-8'>$embedhtml</div>";
                $lightboxgrid .= "<div class='col-sm-4 charteredcollege-lightbox-description'>".
    ...
    private function read_media() {
        ...
        $event->add_record_charteredcollegeshot('resource', $resource);
        $event->trigger();
