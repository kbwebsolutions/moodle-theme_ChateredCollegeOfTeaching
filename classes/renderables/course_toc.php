...
class course_toc implements \renderable, \templatable{
    ...
    use \theme_charteredcollege\output\general_section_trait;
    use \theme_charteredcollege\renderables\trait_exportable;
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            if ($canviewhidden) { // Teachers.
                if ($conditional) {
                    ...
                    $chapter->availabilitystatus = get_string('conditional', 'theme_charteredcollege');
                    $chapter->classes .= 'conditional ';
                }
                if (!$thissection->visible) {
                    $chapter->availabilityclass = 'text-warning';
                    $chapter->availabilitystatus = get_string('notpublished', 'theme_charteredcollege');
            ...
            if ($chapter->title == get_string('general')) {
                $chapter->title = get_string('introduction', 'theme_charteredcollege');
            }

            if ($this->format->is_section_current($section)) {
                $chapter->iscurrent = true;
                $chapter->classes .= 'charteredcollege-visible-section current ';
            }
