...
namespace theme_charteredcollege\calendar\event;

defined('MOODLE_INTERNAL') || die();

use theme_charteredcollege\calendar\event\data_access\event_vault;
use theme_charteredcollege\calendar\event\strategies\activity_retrieval_strategy;
...
/**
 * charteredcollege event container.
 *
 * @copyright 2017 Blackboard Inc
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class container extends \core_calendar\local\event\container {
    ...
    public static function ovd_init() {
        if (empty(self::$eventvault) || get_class(self::$eventvault) != 'theme_charteredcollege\calendar\event\data_access\event_vault') {
        ...
        if (empty(self::$eventvault)) {
            // Use charteredcollege retrieval strategy.
