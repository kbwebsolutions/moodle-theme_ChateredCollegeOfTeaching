...
namespace theme_charteredcollege\services;
...
class course {
    ...
    public function setcoverimage(\context $context, $data, $filename) {
        ...
        if (!in_array($ext, local::supported_coverimage_types())) {
            return ['success' => false, 'warning' => get_string('unsupportedcoverimagetype', 'theme_charteredcollege', $ext)];
        }
        ...
        if (strlen($binary) > get_max_upload_file_size($CFG->maxbytes)) {
            throw new \moodle_exception('error:coverimageexceedsmaxbytes', 'theme_charteredcollege');
        }
        ...
        if ($context->contextlevel === CONTEXT_COURSE) {
            ...
            if (!$this->check_summary_files_for_image_suitability($context)) {
                return ['success' => false, 'warning' => get_string('coursesummaryfilesunsuitable', 'theme_charteredcollege')];
            }
            ...
            $fs->delete_area_files($context->id, 'theme_charteredcollege', 'poster');
        } else {
        ...
        if ($context->contextlevel === CONTEXT_SYSTEM) {
            set_config('poster', $newfilename, 'theme_charteredcollege');
            ...
            if ($context->contextlevel === CONTEXT_COURSECAT) {
                $themecolor = get_config('theme_charteredcollege', 'themecolor');
                $catconfig = get_config('theme_charteredcollege', 'category_color');
                $catscolor = [];
                ...
                if ($catcontrast < 4.5) {
                    return ['success' => true, 'contrast' => get_string('imageinvalidratiocategory',
                        'theme_charteredcollege', number_format((float)$catcontrast, 2))];
                }
            }
            if ($contrast < 4.5) {
                return ['success' => true, 'contrast' => get_string('imageinvalidratio',
                    'theme_charteredcollege', number_format((float)$contrast, 2))];
            }
        }
    ...
    public function course_completion($shortname, $previouslyunavailablesections, $previouslyunavailablemods) {
        ...
        /** @var \theme_charteredcollege_core_course_renderer $courserenderer */
    ...
    public function highlight_section($shortname, $sectionnumber, $highlight) {
        ...
        $actionmodel = new \theme_charteredcollege\renderables\course_action_section_highlight($course, $section);
        $toc = new \theme_charteredcollege\renderables\course_toc($course);
        return [
    ...
    public function set_section_visibility($shortname, $sectionnumber, $visible, $loadmodules = true) {
        ...
        $modinfo = get_fast_modinfo($course);
        $section = $modinfo->get_section_info($sectionnumber);
        $actionmodel = new \theme_charteredcollege\renderables\course_action_section_visibility($course, $section);
