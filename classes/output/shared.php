...
class shared extends \renderer_base {
    ...
    protected static function dndupload_add_to_course($course, $modnames) {
        ...
        $script = <<<EOF
            <script>
                var themecharteredcollegeCourseFileHandlers = $json;
            </script>
EOF;
    ...
    public static function page_requires_js() {
        ...
        $coursevars = (object) [
            ...
            'partialrender' => !empty(get_config('theme_charteredcollege', 'coursepartialrender')),
            'toctype' => get_config('theme_charteredcollege', 'leftnav'),
            'loadPageInCourse' => !empty(get_config('theme_charteredcollege', 'design_mod_page')),
        ];
        ...
        // Bring pre contents scss branding variables, to pass them to charteredcollege init.
        ...
        // Bring grading settings constants with percentage, to pass them to charteredcollege init.
        $gradingconstants = [];
        ...
        if ($alternativelogins->potentialidps) {
            $loginvars = [get_config('theme_charteredcollege', 'enabledlogin'), get_config('theme_charteredcollege', 'enabledloginorder')];
        } else {
            $enabledlogin = \theme_charteredcollege\output\core_renderer::ENABLED_LOGIN_MOODLE;
            $loginvars = [$enabledlogin, null];
        }
        $PAGE->requires->js_call_amd('theme_charteredcollege/charteredcollege', 'charteredcollegeInit', $initvars);
        $PAGE->requires->js_call_amd('theme_charteredcollege/accessibility', 'charteredcollegeAxInit', $initaxvars);
        if (!empty($CFG->calendar_adminseesall) && is_siteadmin()) {
            $PAGE->requires->js_call_amd('theme_charteredcollege/adminevents', 'init');
        }
        $PAGE->requires->js_call_amd('theme_charteredcollege/login_render-lazy', 'loginRender', $loginvars);
    ...
    public static function appendices() {
        ...
        foreach ($instances as $instance) { // Need to check enrolment methods for self enrol.
            if ($instance->enrol === 'self') {
                ...
                if (is_enrolled($coursecontext)) {
                    ...
                    if ($enrolurl) {
                        $selfenrol = true;
                        $enrolstr = get_string('unenrolme', 'theme_charteredcollege');
                        break;
                    }
                } else {
        ...
        if (has_capability('moodle/course:update', $coursecontext)) {
            ...
            $settingsicon = '<img src="'.$iconurl.'" class="charteredcollege-cover-icon svg-icon" alt="" role="presentation">';

            $links[] = array(
                'link' => 'course/edit.php?id='.$COURSE->id,
                'title' => $settingsicon.get_string('editcoursesettings', 'theme_charteredcollege'),
            );
        }
        ...
        if (course_can_view_participants($coursecontext)) {
            ...
            $usercount = \theme_charteredcollege\local::count_enrolled_users($coursecontext, '', 0, true);
            ...
            $participanticons = '<div class="charteredcollege-participant-icons">'.$participanticons.'</div>';
            $links[] = array(
        ...
        if (array_key_exists('pld', $localplugins) && has_capability('local/pld:editcourserules', $coursecontext)) {
            ...
            $pldname = get_string('pld', 'theme_charteredcollege');
            $links[] = array(
        ...
        if ( \core_component::get_component_directory('report_allylti') !== null &&
            ...
            $iconurl = $OUTPUT->image_url('i/ally_logo', 'theme_charteredcollege');
        ...
        $coursetools = get_string('coursetools', 'theme_charteredcollege');
    ...
    public static function print_student_dashboard() {
        ...
        $userpicture->class = 'userpicture charteredcollege-icon'; // Icon class for margin.
        $userpicture->size = 100;
        $userpic = $OUTPUT->render($userpicture);

        $userboard  = '<div id="charteredcollege-student-dashboard" class="row clearfix">';
        ...
        if ($COURSE->enablecompletion) {
            ...
            $userboard .= '<div class="col-xs-3 text-center charteredcollege-student-dashboard-progress">';
            $userboard .= '<h4 class="h6">' .get_string('progress', 'theme_charteredcollege'). '</h6>';
            $userboard .= '<div class="js-progressbar-circle charteredcollege-progress-circle" value="' .round($progress->progress). '"></div>';
            $userboard .= '</div>';
        }
        ...
        if (has_capability('gradereport/overview:view', $coursecontext)) {
            ...
            $userboard .= '<div class="col-xs-3 text-center charteredcollege-student-dashboard-grade">';
            $userboard .= '<h4 class="h6">' . get_string('grade') . '</h6>';
            $userboard .= '<a href="' . $moodleurl . '">';
            $userboard .= '<div class="js-progressbar-circle charteredcollege-progress-circle charteredcollege-progressbar-link" value="';
            $userboard .= s($coursegrade) . '"gradeformat="' . $displayformat . '" ></div>';
            $userboard .= '</a>';
            $userboard .= '</div>';
        }

        $userboard .= '</div><!- close .charteredcollege-user-dashboard ->';
        $userboard .= '<br>';
