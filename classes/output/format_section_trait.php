...
use moodle_url;
use stdClass;
use theme_charteredcollege\renderables\course_action_section_move;
use theme_charteredcollege\renderables\course_action_section_visibility;
use theme_charteredcollege\renderables\course_action_section_delete;
use theme_charteredcollege\renderables\course_action_section_highlight;
use theme_charteredcollege\renderables\course_section_navigation;
...
trait format_section_trait {
    ...
    protected function section_header($section, $course, $onsectionpage, $sectionreturn=null) {
        ...
        // ajax.
        $output = $PAGE->get_renderer('theme_charteredcollege', 'core', RENDERER_TARGET_GENERAL);
        ...
        if ($sectiontitle == get_string('general') && $section->section == 0) {
            $classes = '';
            $sectiontitle = get_string('introduction', 'theme_charteredcollege');
        }

        // Untitled topic title.
        $testemptytitle = get_string('topic').' '.$section->section;
        $leftnav = get_config('theme_charteredcollege', 'leftnav');
        ...
        if ($sectiontitle == $testemptytitle && has_capability('moodle/course:update', $context)) {
            ...
            $o .= "<a href='$url' title='".s(get_string('editcoursetopic', 'theme_charteredcollege'))."'>";
            $o .= get_string('defaulttopictitle', 'theme_charteredcollege')."</a></h2>";
        } else {
            if ($section->section != 0 && $leftnavtop != 0 ) {
        ...
        if ($caneditcourse && $section->section != 0) {
            $extradropclass = !empty(get_config('theme_charteredcollege', 'coursepartialrender')) ? 'partial-render' : '';
            $o .= "<a class='charteredcollege-drop section-drop " . $extradropclass . "' data-title='".
        ...
        if (has_capability('moodle/course:update', $context)) {
            if (!empty($sectiontoolsarray)) {
                ...
                $o .= html_writer::tag('div', $sectiontools, array(
                    'class' => 'js-only charteredcollege-section-editing actions',
                    'role' => 'region',
                    'aria-label' => get_string('topicactions', 'theme_charteredcollege')
                ));
            }
        }
        // Draft message.
        $drafticon = '<img aria-hidden="true" role="presentation" class="svg-icon" src="'.$output->image_url('/i/show').'" />';
        $o .= '<div class="charteredcollege-draft-tag charteredcollege-draft-section">'.$drafticon.' '.get_string('draft', 'theme_charteredcollege').'</div>';

        // Current section message.
        $currenticon = '<img aria-hidden="true" role="presentation" class="svg-icon" src="'.$output->image_url('/i/marked').'" />';
        $o .= '<span class="charteredcollege-current-tag">'.$currenticon.' '.get_string('current', 'theme_charteredcollege').'</span>';
        ...
        if ($formattedinfo !== '') {
            $o .= '<div class="charteredcollege-conditional-tag">'.$conditionalicon.' '.$formattedinfo.'</div>';
        }
        ...
        // Anyways, no course content appears when editing is on when using charteredcollege.
        ...
        if (empty($summarytext) && $canupdatecourse) {
            $summarytext = "<p tabindex='0'>".get_string('defaultsummary', 'theme_charteredcollege')."</p>";
            if ($section->section == 0) {
                $editorname = format_string(fullname($USER));
                $summarytext = "<p tabindex='0'>".get_string('defaultintrosummary', 'theme_charteredcollege', $editorname)."</p>";
            }
        } else {
        ...
        if ($canupdatecourse) {
            ...
            $o .= '<a href="'.$url.'" class="edit-summary">'.$icon.get_string('editcoursetopic', 'theme_charteredcollege'). '</a>';
        }
        $o .= "</div>";
    ...
    public function render_course_section_navigation(course_section_navigation $navigation) {
        return $this->render_from_template('theme_charteredcollege/course_section_navigation', $navigation);
    }
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        $canviewhidden = has_capability('moodle/course:viewhiddensections', context_course::instance($course->id));
        $numsections = course_get_format($course)->get_last_section_number();
        if (get_config('theme_charteredcollege', 'coursepartialrender')) {
            echo $this->render_from_template('theme_charteredcollege/course_section_loading', []);
            $startsectionid = 0;
    ...
    private function change_num_sections($course) {
        ...
        $url = new moodle_url('/theme/charteredcollege/index.php', array(
            'sesskey'  => sesskey(),
            'action' => 'addsection',
        ...
        $required = '';
        $defaulttitle = get_string('title', 'theme_charteredcollege');
        ...
        $heading = get_string('addanewsection', 'theme_charteredcollege');
        $output = "<section id='charteredcollege-add-new-section' class='clearfix' tabindex='-1'>
        <h3>$heading</h3>";
        ...
        $output .= "<label for='newsection' class='sr-only'>".get_string('title', 'theme_charteredcollege')."</label>";
        if ($course->format === 'topics') {
            $output .= '<input class="h3" id="newsection" type="text" maxlength="250" name="newsection" '.$required;
            $output .= ' placeholder="'.s(get_string('title', 'theme_charteredcollege')).'">';
        } else {
            $output .= '<h3>'.$defaulttitle.': '.$datesection.'</h3>';
        }
        $output .= '</div>';
        $output .= '<div class="form-group">';
        $output .= '<label for="summary">'.get_string('contents', 'theme_charteredcollege').'</label>';
        ...
        $output .= html_writer::empty_tag('input', array(
            ...
            'name' => 'addtopic',
            'value' => get_string('createsection', 'theme_charteredcollege'),
        ));
    ...
    public function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        $mcdataattributes = 'data-section="'.$section.'" data-toggle="modal" data-target="#charteredcollege-modchooser-modal"';
        $modchooser = '
        <div class="col-sm-6 charteredcollege-modchooser" id="charteredcollege-create-activity">
            <a href="#" class="'.$mcclass.'" '.$mcdataattributes.'>'.$icon.get_string('addresourceoractivity', 'theme_charteredcollege').'</a>
        </div>';

        // Add zone for quick uploading of files.
        $upload = '<div class="col-sm-6">
            <form class="charteredcollege-dropzone js-only">
                <label tabindex="0" for="charteredcollege-drop-file-'.$section.'" class="charteredcollege-dropzone-label">';
        $upload .= get_string('dropzonelabel', 'theme_charteredcollege').'</label>
                <input type="file" multiple name="charteredcollege-drop-file-'.$section.'" id="charteredcollege-drop-file-'.$section;
        $upload .= '" class="js-charteredcollege-drop-file sr-only"/>
            </form>
            </div>';
    ...
    /**
     * Always output the html for multiple sections, single section mode is not supported in charteredcollege.
     *
     * @param stdClass $course The course entry from DB
    ...
    public function render_course_action_section_move(course_action_section_move $action) {
        ...
        return $this->render_from_template('theme_charteredcollege/course_action_section', $data);
    }
