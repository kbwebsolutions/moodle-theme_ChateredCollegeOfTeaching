...
/**
 * charteredcollege core renderer.
 *
...
use theme_charteredcollege\renderables\course_card;
use theme_charteredcollege\renderables\course_toc;
use theme_charteredcollege\renderables\featured_courses;

// We have to force include this class as it's on login and the auto loader may not have been updated via a cache dump.
require_once($CFG->dirroot.'/theme/charteredcollege/classes/renderables/login_alternative_methods.php');
...
class core_renderer extends \theme_boost\output\core_renderer {
        ...
        if ($helpidentifier) {
            ...
            if ($helpidentifier === 'modulename') {
                ...
                if (!empty($mod) && isset($mod[$component]) && is_object($mod[$component]) && $mod[$component]->help) {
                    ...
                    $collapsablehelp = $this->render_from_template('theme_charteredcollege/heading_help_collapse', $data);
                    $classnames .= ' d-inline';
                }
    ...
    public function column_header_icon_link($langstring, $iconname, $url) {
        global $OUTPUT;
        $text = get_string($langstring, 'theme_charteredcollege');
        $iconurl = $OUTPUT->image_url($iconname, 'theme');
        $icon = '<img class="svg-icon" role="presentation" src="' .$iconurl. '" alt="">';
        $link = '<a class="charteredcollege-personal-menu-more" href="' .$url. '"><small>' .$text. '</small>' .$icon. '</a>';
        return $link;
    }
    ...
    public function mobile_menu_link($langstring, $iconname, $url) {
        global $OUTPUT;
        $alt = get_string($langstring, 'theme_charteredcollege');
        $iconurl = $OUTPUT->image_url($iconname, 'theme');
    ...
    public function render_settings_link(settings_link $settingslink) {
       ...
       $hamburgericon = '<svg xmlns="http://www.w3.org/2000/svg" id="charteredcollege-admin-icon" viewBox="0 0 100 100">
        ...
        $attributes = array(
            ...
            'title' => get_string('admin', 'theme_charteredcollege'),
    ...
    protected function render_message_icon() {
        ...
        if (!empty($CFG->messaging) && !empty(get_config('theme_charteredcollege', 'messagestoggle'))) {
            ...
            $ariaopenmessagedrawer = get_string('openmessagedrawer', 'theme_charteredcollege');
            return '<div class="badge-count-container">
                        <a class="charteredcollege-message-count" href="'.$url.'">
    ...
    protected function render_messages() {
        ...
        $heading = get_string('messages', 'theme_charteredcollege');
        if ($this->advanced_feeds_enabled()) {
            $o = $this->render_feed_web_component('messages', $heading, get_string('nomessages', 'theme_charteredcollege'));
        } else {
            $o = '<h2>'.$heading.'</h2>';
            $o .= '<div id="charteredcollege-personal-menu-messages"></div>';
        }
    ...
    protected function render_forumposts() {
        ...
        $heading = get_string('forumposts', 'theme_charteredcollege');
        if ($this->advanced_feeds_enabled()) {
            ...
            $o = $this->render_feed_web_component('forumposts', $heading,
                            get_string('noforumposts', 'theme_charteredcollege'), $virtualpaging);
        } else {
            $o = '<h2>'.$heading.'</h2>
            <div id="charteredcollege-personal-menu-forumposts"></div>';
        }
     ...
     * @return string
     */
    public function charteredcollege_media_object($url, $image, $title, $meta, $content, $extraclasses = '', $attributes = '') {
        ...
        if (is_array($meta)) {
            $metastr = '<span class="charteredcollege-media-meta">';
            foreach ($meta as $metaitem) {
                $metastr .= $metaitem.'<br>';
            }
            $metastr .= '</span>';
        } else if ($meta) {
            $metastr = '<span class="charteredcollege-media-meta">' .$meta.'</span>';
        }
    ...
    /**
     * Output moodle blocks and charteredcollege wrapper with edit button.
     * @return string
     */
    public function charteredcollege_blocks() {
        ...
        if ($oncoursepage && has_capability('moodle/course:update', $coursecontext)) {
            ...
            if ($PAGE->user_is_editing()) {
                ...
                $url->param('edit', 'on');
                $editstring = get_string('editcoursecontent', 'theme_charteredcollege');
            }
    ...
    private function get_calltoaction_url($key) {
        return '#charteredcollege-personal-menu-' .
    ...
    protected function render_callstoaction() {
        ...
        $mobilemenu = '<div id="charteredcollege-pm-mobilemenu">';
        $mobilemenu .= $this->mobile_menu_link('courses', 'courses', '#charteredcollege-pm-courses');
    ...
    protected function render_grading() {
        ...
        $heading = get_string('grading', 'theme_charteredcollege');
        if ($this->advanced_feeds_enabled()) {
            $virtualpaging = true; // Web service retrieves all elements, need to do virtual paging.
            $o = $this->render_feed_web_component('grading', $heading,
                            get_string('nograding', 'theme_charteredcollege'), $virtualpaging);
        } else {
            $o = "<h2>$heading</h2>";
            $o .= '<div id="charteredcollege-personal-menu-grading"></div>';
        }
    ...
    protected function render_graded() {
        ...
        $heading = get_string('recentfeedback', 'theme_charteredcollege');
        if ($this->advanced_feeds_enabled()) {
            $virtualpaging = true; // Web service retrieves all elements, need to do virtual paging.
            $o = $this->render_feed_web_component('graded', $heading,
                            get_string('nograded', 'theme_charteredcollege'), $virtualpaging);
        } else {
            $o = "<h2>$heading</h2>";
            $o .= '<div id="charteredcollege-personal-menu-graded"></div>';
        }
    ...
    protected function render_deadlines() {
        ...
        $heading = get_string('deadlines', 'theme_charteredcollege');
        if ($this->advanced_feeds_enabled()) {
            $virtualpaging = true; // Web service retrieves all elements, need to do virtual paging.
            $o = $this->render_feed_web_component('deadlines', $heading,
                get_string('nodeadlines', 'theme_charteredcollege'), $virtualpaging);
        } else {
            $o = "<h2>$heading</h2>";
            $o .= '<div id="charteredcollege-personal-menu-deadlines"></div>';
        }
    ...
    public function render_course_card(course_card $card) {
        return $this->render_from_template('theme_charteredcollege/course_cards', $card);
    }
    ...
    public function render_login_alternative_methods(login_alternative_methods $methods) {
        ...
        return $this->render_from_template('theme_charteredcollege/login_alternative_methods', $methods);
    }
    public function render_login_base_method() {
        ...
        return $this->render_from_template('theme_charteredcollege/login_base_methods', $data);
    }
    ...
    public function personal_menu() {
        ...
        if (!isloggedin() || isguestuser()) {
            $enabledlogin = get_config('theme_charteredcollege', 'enabledlogin');
            $enabledloginorder = get_config('theme_charteredcollege', 'enabledloginorder');
            switch ($enabledlogin) {
                default:
            ...
            if ($PAGE->pagetype !== 'login-index') {
                ...
                return $this->render_from_template('theme_charteredcollege/login', $data);
            } else {
                return '';
            }
        }
        ...
        $fullnamelink = '<a href="' .s($CFG->wwwroot). '/user/profile.php"
                    title="' .s(get_string('viewyourprofile', 'theme_charteredcollege')). '"
                    class="h1" aria-level="1" id="charteredcollege-pm-user-profile">'
        ...
        if (\core\session\manager::is_loggedinas()) {
            ...
            $realfullnamelink = '<br>' .get_string('via', 'theme_charteredcollege'). ' ' .format_string(fullname($realuser, true));
        }

        // User quicklinks.
        $profilelink = [
            'id' => 'charteredcollege-pm-profile',
            'link' => s($CFG->wwwroot). '/user/profile.php',
            'title' => get_string('profile')
        ];
        $dashboardlink = [
            'id' => 'charteredcollege-pm-dashboard',
            'link' => s($CFG->wwwroot). '/my',
            'title' => get_string('myhome')
        ];
        $gradelink = [
            'id' => 'charteredcollege-pm-grades',
            'link' => s($CFG->wwwroot). '/grade/report/overview/index.php',
            'title' => get_string('grades')
        ];
        $preferenceslink = [
            'id' => 'charteredcollege-pm-preferences',
        ...
        if (has_capability('moodle/role:switchroles', $coursecontext) || is_role_switched($courseid)) {
            ...
            if (!is_role_switched($courseid)) {
                ...
                $switchrole = [
                    'id' => 'charteredcollege-pm-switchroleto',
                    'link' => $link->out(false),
                ...
                $switchrole = [
                    'id' => 'charteredcollege-pm-switchrolereturn',
                    'link' => $link->out(false),
        ...
        return $this->render_from_template('theme_charteredcollege/personal_menu', $data);
    }
    ...
    public function personal_menu_trigger() {
        ...
        if (!isloggedin() || isguestuser()) {
            ...
            $menu = '<span class="hidden-xs-down">' .get_string('menu', 'theme_charteredcollege'). '</span>';
            $linkcontent = $picture.$menu;
            $attributes = array(
                'aria-haspopup' => 'true',
                'class' => 'js-charteredcollege-pm-trigger charteredcollege-my-courses-menu',
                'id' => 'charteredcollege-pm-trigger',
                'aria-controls' => 'charteredcollege-pm',
            );
    ...
    public function cover_image_selector() {
        ...
        if (has_capability('moodle/course:changesummary', $PAGE->context)) {
            ...
            return $this->render_from_template('theme_charteredcollege/cover_image_selector', $vars);
        }
        return null;
    }
    ...
    public function cover_carousel() {
        ...
        $data['slides'] = $slides;
        return $this->render_from_template('theme_charteredcollege/carousel', $data);
    }
    ...
    public function page_heading($tag = 'h1') {
        ...
        if ($this->page->pagelayout == 'frontpage') {
            $heading .= '<p class="charteredcollege-site-description">' . format_string($this->page->theme->settings->subtitle) . '</p>';
        }
        if ($this->page->user_is_editing() && $this->page->pagelayout == 'frontpage') {
            $url = new moodle_url('/admin/settings.php', ['section' => 'themesettingcharteredcollege']);
            $link = html_writer::link($url,
                            get_string('changefullname', 'theme_charteredcollege'),
    ...
    protected function render_custom_menu(\custom_menu $menu) {
        ...
        $content .= '<ul class="navbar-collapse clearfix" id="charteredcollege-navbar-content">';
        foreach ($menu->get_children() as $item) {
            $context = $item->export_for_template($this);
            $content .= $this->render_from_template('theme_charteredcollege/custom_menu_item', $context);
        }
    ...
    public function site_frontpage_news() {
        ...
        foreach ($discussions as $discussion) {
            ...
            $imgarr = \theme_charteredcollege\local::extract_first_image($message);
            if ($imgarr) {
            ...
            $readmorebtn = "<a class='btn btn-secondary toggle' href='".
                ...
                get_string('readmore', 'theme_charteredcollege')."</a>";
            ...
            if (!$imagestyle) {
                ...
                $newsimage = '<div class="news-article-image toggle"'.$imagestyle.' title="'.
                    get_string('readmore', 'theme_charteredcollege').'"></div>';
            }
...
<div class="news-article clearfix">
    ...
    <div class="news-article-message" tabindex="-1">
        {$message}
        <div><hr><a class="charteredcollege-action-icon charteredcollege-icon-close toggle" href="#">
...
HTML;
        ...
        $actionlinks = html_writer::link(
            ...
            get_string('morenews', 'theme_charteredcollege'),
    ...
    /**
     * add in additional classes that are used for charteredcollege
    ...
    public function body_css_classes(array $additionalclasses = array()) {
        ...
        if (isset($SESSION->justloggedin) && empty($forcepasschange)) {
            ...
            if ($openfixyafterlogin && !isguestuser() && ($onfrontpage || $onuserdashboard)) {
                $classes[] = 'charteredcollege-pm-open';
            }
        }
        ...
        if (!empty($PAGE->theme->settings->resourcedisplay)) {
            $classes[] = 'charteredcollege-resource-'.$PAGE->theme->settings->resourcedisplay;
        } else {
            $classes[] = 'charteredcollege-resource-card';
        }

        // Add theme-charteredcollege class so modules can customise css for charteredcollege.
        $classes[] = 'theme-charteredcollege';

        if (get_config('theme_charteredcollege', 'coursepartialrender') && get_config('theme_charteredcollege', 'leftnav') == 'top'
    ...
    public function confirm($message, $continue, $cancel) {
        ...
        $output = $this->box_start('generalbox charteredcollege-continue-cancel', 'notice');
    ...
    public function feature_spot_cards() {
        ...
        foreach ($fsnames as $feature) {
            ...
            if (!empty($PAGE->theme->settings->$title) && !empty($PAGE->theme->settings->$text)) {
                ...
                if (!empty($PAGE->theme->settings->$image)) {
                    ...
                    $img = '<!--Card image-->
                    <div class="charteredcollege-feature-image-wrap">
                        <img class="charteredcollege-feature-image" src="' .$url. '" alt="" role="presentation">
                    </div>';
                }
        ...
        if ($fscount > 0) {
            ...
            if (!empty($PAGE->theme->settings->fs_heading)) {
                $fstitle = '<h2 class="charteredcollege-feature-spots-heading">' .s($PAGE->theme->settings->fs_heading). '</h2>';
            }
            ...
            foreach ($features as $feature) {
                $cards .= '<div class="' .$colclass. '" id="charteredcollege-feature-' .$i. '">' .$feature. '</div>';
                $i++;
            }

            $fsedit = '';
            if ($this->page->user_is_editing()) {
                $url = new moodle_url('/admin/settings.php?section=themesettingcharteredcollege#themecharteredcollegefeaturespots');
                $link = html_writer::link($url, get_string('featurespotsedit', 'theme_charteredcollege'), ['class' => 'btn btn-primary']);
                $fsedit = '<p class="text-center">'.$link.'</p>';
            }

            // Build feature spots.
            $featurespots = '<div id="charteredcollege-feature-spots">';
    ...
    protected function feature_spot_card($title, $link, $cbopeninnewtab, $image, $text) {
        ...
        // Title with link.
        $linktitle = '<a ' .$target. ' class="charteredcollege-feature-link h5" href="' .s($link). '">' .s($title). '</a>';
        // Title without link.
        $nolinktitle = '<h3 class="charteredcollege-feature-title h5">' .s($title). '</h3>';
        // Content text for feature spots.
        $fscontenttext = '<p class="charteredcollege-feature-text">' .format_text($text). '</p>';

        if ($link) {
            $card = '<div class="charteredcollege-feature">
                        <div class="charteredcollege-feature-block">' .$image.$linktitle.$fscontenttext. '</div>
                    </div>';
        } else {
            $card = '<div class="charteredcollege-feature">
                        <div class="charteredcollege-feature-block">' .$image.$nolinktitle.$fscontenttext. '</div>
                    </div>';
        }
    ...
    public function render_featured_courses(featured_courses $fc) {
        ...
        return $this->render_from_template('theme_charteredcollege/featured_courses', $fc);
    }

    /**
     * Return charteredcollege modchooser modal.
     * @return string
     */
    protected function course_modchooser() {
        ...
        return $this->render_from_template('theme_charteredcollege/course_modchooser_modal', $data);
    }
    ...
    /**
     * Return charteredcollege's logo url for login.mustache
     *
     * @param int $maxwidth not used in charteredcollege.
     * @param int $maxheight not used in charteredcollege.
    ...
    public function get_logo_url($maxwidth = null, $maxheight = 200) {
        ...
        $url = moodle_url::make_file_url("$CFG->httpswwwroot/pluginfile.php", "/$syscontextid/theme_charteredcollege/logo/$itemid".$filepath);
        return $url;
    }
    ...
    protected function render_intelliboard() {
        ...
        $o .= '<div id="charteredcollege-personal-menu-intelliboard">'
                .$links.
                '</div>';
    ...
    protected function render_intellicart() {
        ...
        $o .= '<div id="charteredcollege-personal-menu-intellicart">'
            .$link.
            '</div>';
    ...
    public function navbar($coverimage = '') {
        ...
        $breadcrumbs = '';
        $courseitem = null;
        $attr['class'] = 'js-charteredcollege-pm-trigger';
        $attrs['class'] = '';
        if (!empty($coverimage)) {
            $attr['class'] .= ' mast-breadcrumb';
            $attrs['class'] .= ' mast-breadcrumb';
        }
        $charteredcollegemycourses = html_writer::link('#', get_string('menu', 'theme_charteredcollege'), $attr);
        ...
        foreach ($this->page->navbar->get_items() as $item) {
            ...
            // Replace my courses none-link with link to charteredcollege personal menu.
            if ($item->key === 'mycourses') {
                $breadcrumbs .= '<li class="breadcrumb-item">' .$charteredcollegemycourses. '</li>';
                continue;
            }
                ...
                if ($courseitem != null) {
                    ...
                    if ($COURSE->format == 'topics' || $COURSE->format == 'weeks') {
                        ...
                        if ($item->text == get_string('general')) {
                            $item->text = get_string('introduction', 'theme_charteredcollege');
                        }
                    } else {
    ...
    private function render_feed_web_component($feedkey, $title, $emptymessage, $virtualpaging = false, $showreload = true) {
        global $CFG;
        $pagesize = get_config('theme_charteredcollege', 'personalmenuadvancedfeedsperpage');
        $pagesize = !empty($pagesize) ? $pagesize : 3;
        $maxlifetime = get_config('theme_charteredcollege', 'personalmenuadvancedfeedslifetime');
        $maxlifetime = is_number($maxlifetime) ? $maxlifetime : 30 * MINSECS;
        $sesskey = sesskey();

        $viewmoremsg = get_string('pmadvancedfeed_viewmore', 'theme_charteredcollege');
        $reloadmsg = get_string('pmadvancedfeed_reload', 'theme_charteredcollege');
        ...
        if ((defined('BEHAT_SITE_RUNNING') && BEHAT_SITE_RUNNING)
            ...
            || !empty($CFG->theme_charteredcollege_prepopulate_advanced_feeds)
        ) {
            $initialvalue = htmlspecialchars(json_encode(local::get_feed($feedkey, 0, $pagesize)));
            $initialvalue = "initial-value=\"{$initialvalue}\"";
        }
        return <<<HTML
<charteredcollege-feed elem-id="charteredcollege-personal-menu-feed-{$feedkey}"
           title="{$title}"
           feed-id="{$feedkey}"
