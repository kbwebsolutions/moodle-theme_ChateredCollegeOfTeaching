...
class activity {
    ...
    public static function module_meta(\cm_info $mod) {
        ...
        if (method_exists('theme_charteredcollege\\activity', $methodname)) {
            $meta = call_user_func('theme_charteredcollege\\activity::' . $methodname, $mod);
        } else {
    ...
    protected static function std_meta(\cm_info $mod,
        ...
        $meta->submittedstr = get_string($submitstrkey, 'theme_charteredcollege');
        $meta->notsubmittedstr = get_string('not'.$submitstrkey, 'theme_charteredcollege');
        if (get_string_manager()->string_exists($mod->modname.'draft', 'theme_charteredcollege')) {
            $meta->draftstr = get_string($mod->modname.'draft', 'theme_charteredcollege');
        } else {
            $meta->drafstr = get_string('draft', 'theme_charteredcollege');
        }

        if (get_string_manager()->string_exists($mod->modname.'reopened', 'theme_charteredcollege')) {
            $meta->reopenedstr = get_string($mod->modname.'reopened', 'theme_charteredcollege');
        } else {
            $meta->reopenedstr = get_string('reopened', 'theme_charteredcollege');
        }
        ...
        if (has_capability('mod/assign:grade', $mod->context)) {
            ...
            if (method_exists('theme_charteredcollege\\activity', $methodnsubmissions)) {
                $meta->numsubmissions = call_user_func('theme_charteredcollege\\activity::'.$methodnsubmissions, $courseid, $mod->instance);
            }
            if (method_exists('theme_charteredcollege\\activity', $methodnungraded)) {
                $meta->numrequiregrading = call_user_func('theme_charteredcollege\\activity::'.$methodnungraded, $courseid, $mod->instance);
            }
        } else {
...
-- Exclude those people who can grade quizzes and suspended users
    ...
    public static function get_events(
        ...
        \theme_charteredcollege\calendar\event\container::ovd_init();
        $vault = \theme_charteredcollege\calendar\event\container::get_event_vault();
