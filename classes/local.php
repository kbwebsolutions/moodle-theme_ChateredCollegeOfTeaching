...
class local {
     ...
     * It's used by config.php so that hacks can be targetted at just the charteredcollege theme.
    ...
    protected static function get_cachestamp($key, $cache, $new = false) {
        $key = strval($key);
        $muc = \cache::make('theme_charteredcollege', $cache);
    ...
    public static function course_completion_progress($course) {
        ...
        $muc = \cache::make('theme_charteredcollege', 'course_completion_progress');
    ...
    public static function courseinfo($courseids) {
        ...
        $showgrades = get_config('theme_charteredcollege', 'showcoursegradepersonalmenu');
    ...
    public static function messages() {
        ...
        if (empty($messages)) {
            return '<p class="small">' . get_string('nomessages', 'theme_charteredcollege') . '</p>';
        }
        ...
        foreach ($messages as $message) {
            $o .= $renderer->charteredcollege_media_object(
                $message['actionUrl'],
                $message['iconUrl'],
                $message['title'],
                $message['description'],
    ...
    public static function messages_data($renderhtml = false, $limitfrom = 0, $limitnum = 5, $maxid = -1) {
        ...
        foreach ($messages as $message) {
            ...
            $unreadclass = '';
            if ($message->unread) {
                $unreadclass = ' charteredcollege-unread';
                $meta .= " <span class=charteredcollege-unread-marker>".get_string('unread', 'theme_charteredcollege')."</span>";
            }
    ...
    public static function graded_data($onlyactive = true, $renderhtml = false) {
        ...
        /** @var \theme_charteredcollege\output\core_renderer $output */
        $output = $PAGE->get_renderer('theme_charteredcollege', 'core', RENDERER_TARGET_GENERAL);
        ...
        foreach ($grades as $grade) {
            ...
            $meta = get_string('released', 'theme_charteredcollege', $output->friendly_datetime($releasedon));
    ...
    public static function graded($onlyactive = true) {
        ...
        if (empty($gradedarr)) {
            return '<p class="small">'. get_string('nograded', 'theme_charteredcollege') . '</p>';
        }
        ...
        foreach ($gradedarr as $gradeditem) {
            $o .= $output->charteredcollege_media_object(
    ...
    public static function grading_data($renderhtml = false) {
        ...
        foreach ($grading as $key => $ungraded) {
            ...
            $xungraded = get_string('xungraded', 'theme_charteredcollege', $ungraded->ungraded);

            $function = '\theme_charteredcollege\activity::'.$cm->modname.'_num_submissions';
            ...
            $xofysubmitted = get_string('xofysubmitted', 'theme_charteredcollege', $a);
    ...
    public static function grading() {
        ...
        if (empty($gradingarr)) {
            return '<p class="small">' . get_string('nograding', 'theme_charteredcollege') . '</p>';
        }
    ...
    public static function all_ungraded($userid, $since = null) {
        ...
        $grading = [];
        foreach ($mods as $mod) {
            $class = '\theme_charteredcollege\activity';
    ...
    /**
     * Make url based on file for theme_charteredcollege components only.
     *
     * @param stored_file $file
     * @return \moodle_url | bool
     */
    private static function charteredcollege_pluginfile_url($file) {
        if (!$file) {
            return false;
        } else {
    ...
    public static function course_card_clean_up($context) {
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'theme_charteredcollege', 'coursecard');
    ...
    public static function set_course_card_image($context, $originalfile) {
        if ($originalfile) {
            ...
            $fs = get_file_storage();
            $cardimage = $fs->get_file($context->id, 'theme_charteredcollege', 'coursecard', 0, '/', 'course-card-'.$id.'-'.$filename);
            if ($cardimage) {
                return $cardimage;
            }
            $filespec = array(
                'contextid' => $context->id,
                'component' => 'theme_charteredcollege',
                'filearea' => 'coursecard',
                'itemid' => 0,
                'filepath' => '/',
    ...
    public static function course_card_image_url($courseid) {
        ...
        $fs = get_file_storage();
        $cardimages = $fs->get_area_files($context->id, 'theme_charteredcollege', 'coursecard', 0, "itemid, filepath, filename", false);
        if ($cardimages) {
            ...
            if (stripos($cardimage->get_filename(), 'rawcoverimage.') !== false) {
                ...
                return self::charteredcollege_pluginfile_url($cardimage);
            }
        }
        try {
    ...
    public static function coverimage($context) {
        ...
        $files = $fs->get_area_files($contextid, 'theme_charteredcollege', 'coverimage', 0, "itemid, filepath, filename", false);
        if (!$files) {
            return false;
        }
        if (count($files) > 1) {
    ...
    public static function course_cat_coverimage_url($catid) {
        ...
        return self::charteredcollege_pluginfile_url($file);
    }
    ...
    public static function site_coverimage_original() {
        $theme = \theme_config::load('charteredcollege');
        ...
        if ($filename) {
            ...
            $fullpath = '/'.$syscontextid.'/theme_charteredcollege/poster/0'.$filename;
            $fs = get_file_storage();
    ...
    public static function process_coverimage(\context $context, $originalfile = false) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'theme_charteredcollege', 'coverimage');
    ...
    public static function render_recent_forum_activity() {
        ...
        if (empty($activities)) {
            return '<p class="small">' . get_string('noforumposts', 'theme_charteredcollege') . '</p>';
        }
    ...
    public static function add_calendar_change_stamp($courseid) {
        $muc = \cache::make('theme_charteredcollege', 'generalstaticappcache');
    ...
    public static function get_profile_based_branding_class($user) {
        ...
        if (empty(get_config('theme_charteredcollege', 'pbb_enable')) || !isloggedin()) {
            return false;
        }

        $cache = \cache::make('theme_charteredcollege', 'profile_based_branding');
        $class = $cache->get('pbb_class');
        if (!empty($class)) {
            return $class;
        }

        $pbbfield = get_config('theme_charteredcollege', 'pbb_field');
...
SQL;
        ...
        if (!empty($value)) {
            $class = 'charteredcollege-pbb-' . self::slugify($value);
    ...
    public static function deadlines() {
        global $PAGE, $USER;
        $eventsobj = \theme_charteredcollege\activity::upcoming_deadlines($USER->id);

        $events = self::deadlines_data($eventsobj, true);
        $fromcache = $eventsobj->fromcache ? 1 : 0;
        $datafromcache = ' data-from-cache="'.$fromcache.'" ';
        if (empty($events)) {
            return '<p class="small"'.$datafromcache.'>' . get_string('nodeadlines', 'theme_charteredcollege') . '</p>';
        }
        ...
        foreach ($events as $event) {
            if (!empty($event->modulename)) {
                ...
                if (!has_capability('mod/assign:grade', $cm->context)) {
                    ...
                    $meta .= '<div class="charteredcollege-completion-meta">' .
                        \theme_charteredcollege\output\core\course_renderer::submission_cta($cm, $activitymeta) .
                        '</div>';
                }
    ...
    public static function get_feed(string $feedid, $page = 0, $pagesize = 3, $maxid = -1) : array {
        ...
        switch ($feedid) {
            ...
            case 'messages':
                ...
                break;
            case 'deadlines':
                $limit = !empty($CFG->charteredcollege_advanced_feeds_max_deadlines) ? $CFG->charteredcollege_advanced_feeds_max_deadlines : 5;
    ...
    /**
     * This Validates if the settings are being shown on charteredcollege personal menu.
     */
