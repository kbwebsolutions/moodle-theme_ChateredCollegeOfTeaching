...
/**
 * charteredcollege page requirements manager.
...
class charteredcollege_page_requirements_manager extends \page_requirements_manager {
    /**
     ...
     * the charteredcollege_page_requirements_manager.
     */
    public function copy_page_requirements() {
        ...
        foreach ($props as $prop) {
            ...
            $charteredcollegemanprop = new \ReflectionProperty($this, $pname);
            // If the property is private or protected  set accessible, after the copy reset to not accessible.
            $isprotected = $charteredcollegemanprop->isPrivate() || $charteredcollegemanprop->isProtected();
            if ($isprotected) {
                $charteredcollegemanprop->setAccessible(true);
            }
            $charteredcollegemanprop->setValue($this, $pval);
            $charteredcollegemanprop->setAccessible(!$isprotected);
        }
