...
$themeischarteredcollege = $theme === 'charteredcollege';
$notajaxscript = !defined('AJAX_SCRIPT') || AJAX_SCRIPT == false;
// The code inside this conditional block is to be executed prior to page rendering when the theme is set to charteredcollege and
...
// workaround.
if ($themeischarteredcollege && $notajaxscript) {

    // Setup debugging html.
    // This allows javascript to target debug messages and move them to footer.
    if (!empty($CFG->charteredcollegewrapdebug) && !function_exists('xdebug_break')) {
    ...
    if (empty($SESSION->theme_charteredcollege_last_course) || $SESSION->theme_charteredcollege_last_course != $COURSE->id) {
        $USER->editing = 0;
        $SESSION->theme_charteredcollege_last_course = $COURSE->id;
    }

    if (isset($SESSION->wantsurl)) {
        // We are taking a backup of this because it can get unset later by core.
        $SESSION->charteredcollegewantsurl = $SESSION->wantsurl;
    }
}
...
if ($themeischarteredcollege && $notajaxscript) {
    if (empty($CFG->charteredcollegepageinit) && !empty($PAGE)) {
        $CFG->charteredcollegepageinit = true;
        $PAGE->initialise_theme_and_output();

        // Modify $PAGE to use charteredcollege requirements manager.
        $charteredcollegepman = new charteredcollege_page_requirements_manager();
        $charteredcollegepman->copy_page_requirements();
    }
