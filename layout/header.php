...
\theme_charteredcollege\output\shared::page_requires_js();
...
if ($PAGE->context->contextlevel === CONTEXT_COURSECAT) {
    if ($PAGE->pagelayout === 'coursecategory') {
        $coverimagecss = \theme_charteredcollege\local::course_cat_coverimage_css($PAGE->context->instanceid);
    }
} else if ($PAGE->pagelayout === 'frontpage' || $PAGE->pagelayout === 'login') {
    $coverimagecss = \theme_charteredcollege\local::site_coverimage_css();
} else {
    $coverimagecss = \theme_charteredcollege\local::course_coverimage_css($COURSE->id);
}
