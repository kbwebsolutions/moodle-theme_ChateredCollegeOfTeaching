 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_charteredcollege extends behat_base {
    ...
    protected function upload_file($fixturefilename, $selector) {
        ...
        $filepath = $CFG->dirroot.'/theme/charteredcollege/tests/fixtures/'.$fixturefilename;
        $file = $this->find('css', $selector);
        $file->attachFile($filepath);
    }

    /**
     * @param string $fixturefilename this is a filename relative to the charteredcollege fixtures folder.
     * @param int $section
     *
    ...
    public function i_upload_file($fixturefilename, $section = 1) {
        $this->upload_file($fixturefilename, '#charteredcollege-drop-file-'.$section);
    }
    ...
    public function i_upload_cover_image($fixturefilename) {
        $this->upload_file($fixturefilename, '#charteredcollege-coverfiles');
        $this->getSession()->executeScript('jQuery( "#charteredcollege-coverfiles" ).trigger( "change" );');
    }
    ...
    public function i_create_a_new_section_in_course_with_content($shortname) {
        ...
        $fs->create_file_from_pathname($fileinfo, $CFG->dirroot . "/theme/charteredcollege/tests/fixtures/testpng.png");
    ...
    public function i_open_the_personal_menu() {
        $node = $this->find('css', '#charteredcollege-pm');
        // Only attempt to open the personal menu if its not already open.
        if (!$node->isVisible()) {
            // @codingStandardsIgnoreLine
            /* @var $generalcontext behat_general */
            $generalcontext = behat_context_helper::get('behat_general');
            $generalcontext->i_click_on('.charteredcollege-my-courses-menu', 'css_element');
        }
    }
    ...
    public function i_close_the_personal_menu() {
        ...
        if ($node->isVisible()) {
            $node = $this->find('css', '#charteredcollege-pm-close');
            $node->click();
        }
    }
    ...
    public function i_see_availabilityinfo($str, $baseselector = '') {
        $str = trim($str);
        $nodes = $this->find_all('xpath', $baseselector.'//div[contains(@class, \'charteredcollege-conditional-tag\')]');
    ...
    private function base_selector_availabilityinfo($type, $elementstr) {
        if ($type === 'section') {
            ...
            $baseselector = '(//li[contains(@class, \'charteredcollege-asset\')]'. // Selection when editing teacher.
                '//h3[contains(@class, \'charteredcollege-asset-link\')]'.
                '//span[contains(text(), \''.$elementstr.'\')]'.
                '/parent::a/parent::h3/parent::div'.
                '|'.
                '//li[contains(@class, \'charteredcollege-asset\')]'. // Selection when anyone else.
                '//h3[contains(@class, \'charteredcollege-asset-link\')]'.
    ...
    public function i_should_see_available_from_in_asset($date, $nthasset, $section) {
        ...
        $elementselector = '#section-'.$section.' li.charteredcollege-asset:nth-of-type('.$nthasset.')';
    ...
    public function i_should_see_available_from_in_section($date, $section) {
        $elementselector = '#section-'.$section.' > div.content > .charteredcollege-conditional-tag';
        return $this->i_should_see_available_from_in_element($date, $elementselector, 'css_element');
    }
    ...
    public function course_is_favorited($shortname) {
        ...
        /* @var behat_general $general */
        $general = behat_context_helper::get('behat_general');
        $this->ensure_element_does_not_exist('.charteredcollege-icon-toggle.favoritetoggle.ajaxing', 'css_element');
    ...
    public function i_follow_in_the_mobile_menu($link) {
        $node = $this->get_node_in_container('link', $link, 'css_element', '#charteredcollege-pm-mobilemenu');
    ...
    protected function coursecard_image() {
        $session = $this->getSession();
        return $session->evaluateScript(
            "return jQuery('#charteredcollege-pm-courses-current-cards div[style*=\'background-image: url\']').css('background-image')"
        );
    }
    ...
    public function the_course_format_is_set_to($shortname, $format) {
        global $DB;
        $service = theme_charteredcollege\services\course::service();
    ...
    public function i_highlight_section($section) {
        $xpath = '//li[@id="section-'.$section.'"]//div[contains(@class, "charteredcollege-section-editing")]';
        $xpath .= '//a[contains(@class, "charteredcollege-highlight")][@aria-pressed="false"]';
        $this->execute('behat_general::i_click_on', [$xpath, 'xpath_element']);
    }
    ...
    private function personal_menu_deadline_xpath($deadline, $eventname) {
        ...
        $ids = "@id='charteredcollege-personal-menu-deadlines'|@id='charteredcollege-personal-menu-feed-deadlines'";
    ...
    private function meta_assign_xpath($name) {
        ...
        "/parent::div//div[contains(@class, 'charteredcollege-completion-meta')]";
        return $xpath;
    }
    ...
    public function meta_assign_overdue($name) {
        ...
        $xpath .= "/a[contains(@class, 'charteredcollege-due-date')][contains(@class, 'tag-danger')]";
        $this->ensure_element_is_visible($xpath, 'xpath_element');
    }
    ...
    public function meta_assign_not_overdue($name) {
        $xpath = $this->meta_assign_xpath($name);
        $xpath .= "/a[contains(@class, 'charteredcollege-due-date')][contains(@class, 'tag-success')]";
        $this->ensure_element_is_visible($xpath, 'xpath_element');
        $xpath = $this->meta_assign_xpath($name);
        $xpath .= "/a[contains(@class, 'charteredcollege-due-date')][contains(@class, 'tag-danger')]";
        $this->ensure_element_does_not_exist($xpath, 'xpath_element');
    ...
    /**
     * Purge charteredcollege caches.
     *
     * @Given /^I purge charteredcollege caches$/
     */
    public function i_purge_charteredcollege_caches() {
