...
#
# Tests for charteredcollege personal menu on mobile devices.
#
...
Feature: When the moodle theme is set to charteredcollege, students and teachers can open a personal menu which works responsively
  on mobile devices.
  ...
  Scenario Outline: Teacher / Student can view personal menu on a mobile device.
    ...
    And I click on "#charteredcollege-pm-close" "css_element"
    And I open the personal menu
    And I wait until "#charteredcollege-pm-mobilemenu" "css_element" is visible
  ...
  Scenario Outline: Mobile menu icons only appear when enabled.
    Given I change window size to "320x480"
    And the following config values are set as admin:
      | <toggle> | 0 | theme_charteredcollege |
    And I log in as "student1"
    And I open the personal menu
    Then "a[href='<href>']" "css_element" should not exist
    And the following config values are set as admin:
      | <toggle> | 1 | theme_charteredcollege |
    And I reload the page
    And I open the personal menu
    Then "a[href='<href>']" "css_element" should exist
    Examples:
    | toggle           | href                           |
    | deadlinestoggle  | #charteredcollege-personal-menu-deadlines  |
    | feedbacktoggle   | #charteredcollege-personal-menu-graded     |
    | messagestoggle   | #charteredcollege-personal-menu-messages   |
    | forumpoststoggle | #charteredcollege-personal-menu-forumposts |
