...
use \theme_charteredcollege\activity,
    \theme_charteredcollege\charteredcollege_base_test;
...
/**
 * Testing for theme/charteredcollege/classes/activity.php
 *
 * @package  theme_charteredcollege
 * @copyright  2017 Blackboard Inc.
 * @license  http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_charteredcollege_acitvity_test extends charteredcollege_base_test {
    ...
    public function test_get_calendar_activity_events() {
        ...
        $charteredcollegeevent = reset($events);
        $this->assertEquals($due, $charteredcollegeevent->timestart);
        $this->assertEquals($due, $charteredcollegeevent->timesort);

        // Test extension set.
        $this->extend_assign_deadline($assign->id, $student->id, $extension);
        // Test that the charteredcollege calendar includes extensions to assignments.
        $events = activity::get_calendar_activity_events($tstart, $tend, $courses);
        $charteredcollegeevent = reset($events);
        $this->assertEquals($extension, $charteredcollegeevent->timestart);
        $this->assertEquals($extension, $charteredcollegeevent->timesort);
        ...
        // Test that the charteredcollege calendar event is the same as the core event
        // (with the exception of the timestart + timesort fields).
        unset($charteredcollegeevent->timestart);
        unset($charteredcollegeevent->timesort);
        unset($coreevent->timestart);
        unset($coreevent->timesort);
        $this->assertEquals($charteredcollegeevent, $coreevent);
    }
    ...
    public function test_user_activity_events() {
        ...
        // Assert from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEmpty($charteredcollegeevent);
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovdgroupdue, $charteredcollegeevent->timestart);
        $this->assertEquals($ovdgroupdue, $charteredcollegeevent->timesort);
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovduserdue, $charteredcollegeevent->timestart);
        $this->assertEquals($ovduserdue, $charteredcollegeevent->timesort);
    ...
    public function test_instance_activity_dates_quiz_assign() {
        ...
        $this->setUser($student);
        $quizdates = \theme_charteredcollege\activity::instance_activity_dates($course->id, $cm, '', '');
        ...
        $dates1 = \theme_charteredcollege\activity::instance_activity_dates($course->id, $cm, '', '');
        $dates2 = \theme_charteredcollege\activity::instance_activity_dates($course->id, $cm2, '', '');
        ...
        $this->setUser($student);
        $assigndates = \theme_charteredcollege\activity::instance_activity_dates($course->id, $cm, '', '');
    ...
    public function test_hash_events_by_module_instance() {
        ...
        $events = phpunit_util::call_internal_method(null, 'hash_events_by_module_instance', [$events],
            '\theme_charteredcollege\activity');
    ...
    public function test_charteredcollege_deadlines_feed_size() {
        global $CFG;
        ...
        $CFG->charteredcollege_advanced_feeds_max_deadlines = 10;
        $deadlines = local::get_feed('deadlines');
