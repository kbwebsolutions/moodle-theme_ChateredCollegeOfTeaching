...
namespace theme_charteredcollege\tests;

use theme_charteredcollege\local;
use theme_charteredcollege\renderables\course_card;
use theme_charteredcollege\charteredcollege_base_test;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_charteredcollege_local_test extends charteredcollege_base_test {
    ...
    public function test_resolve_theme() {
        ...
        $this->assertEquals('charteredcollege', $theme);
     ...
     * charteredcollege theme settings page. Creates a file, sets a theme
    ...
    protected function fake_site_image_setting_upload($filename) {
        ...
        $filepath = $CFG->dirroot.'/theme/charteredcollege/tests/fixtures/'.$filename;

        $fs = \get_file_storage();

        $fs->delete_area_files($syscontext->id, 'theme_charteredcollege', 'poster');

        $fs->create_file_from_pathname($filerecord, $filepath);
        \set_config('poster', $filename, 'theme_charteredcollege');
     ...
     * charteredcollege theme settings page. Deletes a file, sets a theme
    ...
    protected function fake_site_image_setting_cleared() {
        ...
        $fs->delete_area_files($syscontext->id, 'theme_charteredcollege', 'coverimage');

        \set_config('poster', '', 'theme_charteredcollege');
        local::process_coverimage($syscontext);
    }
    ...
    public function test_poster_image_upload() {
        ...
        foreach ($fixtures as $filename => $shouldberesized) {
            ...
            $this->assertContains('/theme_charteredcollege/coverimage/', $css);
    ...
    protected function fake_course_image_setting_upload($filename, $context) {
        ...
        $fs->create_file_from_pathname($filerecord, $filepath);
        \set_config('coverimage', $filename, 'theme_charteredcollege');
    }
    ...
    public function test_course_grade() {
        ...
        set_config('showcoursegradepersonalmenu', 1, 'theme_charteredcollege');
        ...
        set_config('showcoursegradepersonalmenu', 0, 'theme_charteredcollege');
        $feedback = local::course_grade($course);
        $this->assertTrue(empty($feedback->coursegrade));
    }
    ...
    public function test_get_profile_based_branding() {
        ...
        \set_config('pbb_enable', '1', 'theme_charteredcollege');
        ...
        $this->assertEquals('charteredcollege-pbb-marketing', local::get_profile_based_branding_class($user));
        ...
        $this->assertEquals('charteredcollege-pbb-super-enterprise-sales', local::get_profile_based_branding_class($user));
        ...
        \set_config('pbb_field', 'profile|' . $fieldid, 'theme_charteredcollege');
        local::clean_profile_based_branding_cache();

        // Custom field is set as new value.
        $this->assertEquals('charteredcollege-pbb-banana-split', local::get_profile_based_branding_class($user));
    }
    ...
    public function test_clean_course_card_bg_image_cache() {
        ...
        $cache = \cache::make('theme_charteredcollege', 'course_card_bg_image');
    ...
    public function test_charteredcollege_compare_colors() {
