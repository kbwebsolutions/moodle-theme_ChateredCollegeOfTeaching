...
/**
 * Testcase for charteredcollege theme privacy implementation.
 *
...
use theme_charteredcollege\privacy\provider;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_charteredcollege_privacy_provider_testcase extends provider_testcase {
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new approved_contextlist($user1, 'theme_charteredcollege', [$user1context->id]);
        provider::export_user_data($approvedlist);

        $data = $writer->get_data(['theme_charteredcollege-course-favorites']);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertEquals(4, $DB->count_records('theme_cct_course_favorites', []));

        $user1context = \context_user::instance($user1->id);
        provider::delete_data_for_all_users_in_context($user1context);
        $this->assertEquals(0, $DB->count_records('theme_cct_course_favorites', ['userid' => $user1->id]));
        $this->assertEquals(2, $DB->count_records('theme_cct_course_favorites', ['userid' => $user2->id]));

        $user2context = \context_user::instance($user2->id);
        provider::delete_data_for_all_users_in_context($user2context);

        $this->assertEquals(0, $DB->count_records('theme_cct_course_favorites', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $user1context = \context_user::instance($user1->id);
        $approvedcontextlist = new approved_contextlist($user1, 'theme_charteredcollege', [$user1context->id]);
        ...
        $user2context = \context_user::instance($user2->id);
        $approvedcontextlist = new approved_contextlist($user2, 'theme_charteredcollege', [$user2context->id]);
        provider::delete_data_for_user($approvedcontextlist);
    ...
    private function create_favorites($favorites) {
        ...
        $DB->insert_records('theme_cct_course_favorites', $favorites);
    }
    ...
    public function test_delete_data_for_users() {
        ...
        $this->create_favorites($favorites);
        $this->assertEquals(2, $DB->count_records('theme_cct_course_favorites'));

        $approveduserlist = new \core_privacy\local\request\approved_userlist($context, 'core_course', [$user1->id]);
        provider::delete_data_for_users($approveduserlist);
        $this->assertEquals(1, $DB->count_records('theme_cct_course_favorites', []));
