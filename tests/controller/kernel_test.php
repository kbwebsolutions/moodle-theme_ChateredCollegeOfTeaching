...
class kernel_test extends \basic_testcase {
    ...
    public function test_resolve_controller_callback() {
        $controller = $this->createPartialMock('\theme_charteredcollege\controller\controller_abstract', array(
            'init',
            'test_action',
            'require_capability',
        ));

        $router = $this->createPartialMock('\theme_charteredcollege\controller\router', array('route_action'));
