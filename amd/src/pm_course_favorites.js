 ...
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/log', 'theme_charteredcollege/model_view', 'theme_charteredcollege/ajax_notification'],
    function($, ajax, notification, log, mview, ajaxNotify) {
        return function() {
            ...
            var reloadCourseCardTemplate = function(renderable, cardEl) {
                var dfd = $.Deferred();
                mview(cardEl, 'theme_charteredcollege/course_cards');
            ...
            var moveOutOfFavorites = function(cardEl, onMoveComplete) {
                ...
                // When this is 0 we only have hidden courses, so container is #charteredcollege-pm-courses-current-cards.
                var publishedcount = $('#charteredcollege-pm-courses-current .coursecard:not([data-hidden="true"])').length;
                // Special stuff for when moving a hidden course.
                if ($(cardEl).data('hidden') === true && publishedcount > 0) {
                    container = '#charteredcollege-pm-courses-hidden-cards';
                    // Open hidden courses section.
                    $('#charteredcollege-pm-courses-hidden').addClass('state-visible');
                    $('#charteredcollege-pm-courses-hidden-cards').collapse('show');
                } else {
                    window.console.log('not a hidden card');
                    container = '#charteredcollege-pm-courses-current-cards';
                }
            ...
            var favoriteCourse = function(button) {
                ...
                var doAjax = function(jsid) {
                    return ajax.call([
                        {
                            methodname: 'theme_charteredcollege_course_card',
                ...
                if (favorited === 1) {
                    ...
                    // Move to favorites.
                    moveCard(cardEl, '#charteredcollege-pm-courses-current-cards .coursecard.favorited', '#charteredcollege-pm-courses-current-cards', true,
                        function() {
                            doAjax(jsid);
                        }
                    );
             ...
             */
            $("#charteredcollege-pm").on("click", ".favoritetoggle", function(e) {
                e.preventDefault();
                e.stopPropagation();
                favoriteCourse(this);
            });
