...
/* exported charteredcollegeInit */
/* eslint no-invalid-this: "warn"*/

/**
 * Main charteredcollege initialising function.
 */
define(['jquery', 'core/log', 'theme_charteredcollege/headroom', 'theme_charteredcollege/util', 'theme_charteredcollege/personal_menu',
        'theme_charteredcollege/cover_image', 'theme_charteredcollege/progressbar', 'core/templates', 'core/str', 'theme_charteredcollege/accessibility',
        'theme_charteredcollege/messages'],
    function($, log, Headroom, util, personalMenu, coverImage, ProgressBar, templates, str, accessibility, messages) {

        'use strict';

        /* eslint-disable camelcase */
        M.theme_charteredcollege = M.theme_charteredcollege || {};
         ...
         * and being updated from php constants in charteredcollegeInit.
         */
        ...
        var mobileFormChecker = function() {
            var savebuttonsformrequired = $('div[role=main] .mform div.charteredcollege-form-required fieldset > div.form-group.fitem');
            var savebuttonsformadvanced = $('div[role=main] .mform div.charteredcollege-form-advanced > div:nth-of-type(3)');
            var width = $(window).width();
            if (width < 767) {
                $('.charteredcollege-form-advanced').append(savebuttonsformrequired);
            } else if (width > 767)  {
                $('.charteredcollege-form-required fieldset#id_general').append(savebuttonsformadvanced);
            }
        };
        ...
        var movePHPErrorsToHeader = function() {
            ...
            if (msgs.length) {
                ...
                var errorcont = $('<div id="footer-error-cont"><h3>' +
                    M.util.get_string('debugerrors', 'theme_charteredcollege') +
                    '</h3><hr></div>');
                ...
                var errorlink = $('<a class="footer-error-link btn btn-danger" href="#footer-error-cont">' +
                    M.util.get_string('problemsfound', 'theme_charteredcollege') + ' <span class="badge">' + (msgs.length) + '</span></a>');
        ...
        var setForumStrings = function() {
            $('.path-mod-forum tr.discussion td.topic.starter').attr('data-cellname',
                M.util.get_string('forumtopic', 'theme_charteredcollege'));
            $('.path-mod-forum tr.discussion td.picture:not(\'.group\')').attr('data-cellname',
                M.util.get_string('forumauthor', 'theme_charteredcollege'));
            $('.path-mod-forum tr.discussion td.picture.group').attr('data-cellname',
                M.util.get_string('forumpicturegroup', 'theme_charteredcollege'));
            $('.path-mod-forum tr.discussion td.replies').attr('data-cellname',
                M.util.get_string('forumreplies', 'theme_charteredcollege'));
            $('.path-mod-forum tr.discussion td.lastpost').attr('data-cellname',
                M.util.get_string('forumlastpost', 'theme_charteredcollege'));
        };
        ...
        var bodyClasses = function() {
            var extraclasses = [];
            if ($('#notice.charteredcollege-continue-cancel').length) {
            ...
            $(window).on('popstate hashchange', function(e) {
                ...
                if (newHash !== lastHash) {
                    if (location.hash === '#primary-nav') {
                        ...
                        $('#page, #moodle-footer, #js-charteredcollege-pm-trigger, #logo, .skiplinks').css('display', '');
                        if (onCoursePage()) {
        ...
        var recentUpdatesFix = function() {
            $('#charteredcollege-course-footer-recent-activity .info').each(function() {
                $(this).appendTo($(this).prev());
            });
            $('#charteredcollege-course-footer-recent-activity .head .name').each(function() {
        ...
        var progressbarcircle = function() {
            $('.charteredcollege-student-dashboard-progress .js-progressbar-circle').each(function() {
                createColoredDataCircle(this, function(nodePointer) {
                    return $(nodePointer).attr('value') + '<small>%</small>';
                });
            });

            $('.charteredcollege-student-dashboard-grade .js-progressbar-circle').each(function() {
                createColoredDataCircle(this, function(nodePointer) {
        ...
        var addListeners = function() {
            ...
            var onPin = () => {
                $('.charteredcollege-drawer-no-headroom').addClass('charteredcollege-drawer-headroom');
                $('.charteredcollege-drawer-headroom').removeClass('charteredcollege-drawer-no-headroom');
            };
            var onUnpin = () => {
                $('.charteredcollege-drawer-headroom').addClass('charteredcollege-drawer-no-headroom');
                $('.charteredcollege-drawer-no-headroom').removeClass('charteredcollege-drawer-headroom');
            };
        ...
        return {
            /**
             * charteredcollege initialise function.
             ...
             */
            charteredcollegeInit: function(courseConfig, pageHasCourseContent, siteMaxBytes, forcePassChange,
                ...
                M.charteredcollegeTheme = {forcePassChange: forcePassChange};
                ...
                if (pageHasCourseContent) {
                    require(
                        [
                            'theme_charteredcollege/course-lazy'
                ...
                $(document).ready(function() {
                    ...
                    $('.charteredcollege-resource-long .contentafterlink .charteredcollege-resource-card-fadeout').each(function() {
                    ...
                    $('.charteredcollege-header-card .charteredcollege-header-card-icons .disabled-charteredcollege-asset-completion-tracking').remove();

                    // Prepend asset type when activity is a folder to appear in the card header instead of the content.
                    var folders = $('li.charteredcollege-activity.modtype_folder');
                    ...
                    $('li.charteredcollege-activity.modtype_folder td[id^="ygtvcontentel"] > div > span.fp-filename').each(function () {
                        $(this).replaceWith("<h3>"+$(this).text()+"</h3>");
                    });

                    // Add a class to the body to show js is loaded.
                    $('body').addClass('charteredcollege-js-loaded');
                    ...
                    // Allow deeplinking to bs tabs on charteredcollege settings page.
                    if ($('#page-admin-setting-themesettingcharteredcollege').length) {
                        ...
                        var advFeedsCheckboxSelector = '#id_s_theme_charteredcollege_personalmenuadvancedfeedsenable';
                    ...
                    var firstlinkerror = $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots' +
                        ' #admin-fs_one_title_link span.error');
                    var secondlinkerror = $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots' +
                        ' #admin-fs_two_title_link span.error');
                    var thirdlinkerror = $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots' +
                        ' #admin-fs_three_title_link span.error');
                    var titlelinksettingone = $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots' +
                        ' #admin-fs_one_title_link .form-label');
                    var titlelinksettingtwo = $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots' +
                        ' #admin-fs_two_title_link .form-label');
                    var titlelinksettingthree = $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots' +
                        ' #admin-fs_three_title_link .form-label');
                    // Create an extra Div to wrap title links settings to avoid line break.
                    $('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots ' +
                        '#admin-fs_three_title').nextUntil('#page-admin-setting-themesettingcharteredcollege #themecharteredcollegefeaturespots ' +
                    ...
                    if ($('body').hasClass('charteredcollege-pm-open')) {
                        personalMenu.update();
                    }
                    ...
                    if ((onModSettings || onCourseSettings || onSectionSettings) && pageNotInBlacklist) {
                        ...
                        $('form[id^="mform1"] > fieldset').not(vital).wrapAll('<div class="charteredcollege-form-advanced col-md-4" />');

                        // Add expand all to advanced column.
                        $(".charteredcollege-form-advanced").append($(".collapsible-actions"));
                        // Add collapsed to all fieldsets in advanced, except on course edit page.
                        if (!$('#page-course-edit').length) {
                            $(".charteredcollege-form-advanced fieldset").addClass('collapsed');
                        }
                        ...
                        $(mainForm).wrap('<div class="charteredcollege-form-required col-md-8" />');
                        ...
                        if (onModSettings && description) {
                            ...
                            if ($(addMultiMessageSelectors.join()).length > 0) {
                                str.get_strings([
                                    {key : 'multimediacard', component : 'theme_charteredcollege'}
                        ...
                        $("#page-mod-resource-mod .charteredcollege-form-advanced #id_modstandardelshdr .fcontainer").append(description);
                        $("#page-mod-resource-mod .charteredcollege-form-advanced #id_modstandardelshdr .fcontainer").append(showdescription);
                        ...
                        $("#page-mod-assign-mod .charteredcollege-form-advanced #id_modstandardelshdr .fcontainer").append(filemanager);
                        $("#page-mod-assign-mod .charteredcollege-form-required .fcontainer").append(duedate);

                        // Move availablity at the top of advanced settings.
                        var availablity = $('#id_visible').closest('.form-group').addClass('charteredcollege-form-visibility');
                        ...
                        $(label).text(M.util.get_string('visibility', 'theme_charteredcollege') + ' ');
                        ...
                        if ($("#page-course-edit").length) {
                            ...
                            (function(){
                                return str.get_strings([
                                    {key: 'showallsectionsdisabled', component: 'theme_charteredcollege'},
                                    {key: 'disabled', component: 'theme_charteredcollege'}
                                ]);
                            })()
                                .then(function(strings) {
                                    var strMessage = strings[0];
                                    strDisabled = strings[1];
                                    return templates.render('theme_charteredcollege/form_alert', {
                                        type: 'warning',
                                        classes: '',
                                        message: strMessage
                                    });
                        ...
                        $('.charteredcollege-form-advanced').prepend(availablity);
                        ...
                        if ($("#page-mod-page-mod").length) {
                            ...
                            (function() {
                                return str.get_strings([
                                    {key: 'showappearancedisabled', component: 'theme_charteredcollege'}
                                ]);
                            })()
                    ...
                    if (messageBadgeCountEnabled) {
                        require(
                            [
                                'theme_charteredcollege/conversation_badge_count-lazy'
                    ...
                    var focustarget = $('#charteredcollege-coverimagecontrol label');
                    if (focustarget && focustarget.length) {
                        focustarget.keypress(function (e) {
                            if (e.which === 13) {
                                $('#charteredcollege-coverfiles').trigger('click');
                            }
                        });
                    }
                    ...
                    if (!$('.block_settings').length) {
                        ...
                        if (inAlternativeRole) {
                            ...
                            require(
                                [
                                    'theme_charteredcollege/alternative_role_handler-lazy'
                    ...
                    var customMenu = $('ul#charteredcollege-navbar-content li:first-child a');
                    ...
                    if (notificationsBtn.length && searchButton.length && adminMenuTrigger.length && lastElement.length) {
                        ...
                        require(
                            [
                                'theme_charteredcollege/rearrange_tab_handler-lazy'
                    ...
                    $('.charteredcollege-settings-tab-link').on('click', function() {
                ...
                accessibility.charteredcollegeAxInit();
                messages.init();
            }
        };
    }
