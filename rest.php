...
use theme_charteredcollege\controller\kernel;
...
$PAGE->set_url('/theme/charteredcollege/rest.php', array('action' => $action, 'contextid' => $context->id));
...
foreach ($contfiles as $contfile) {
    ...
    if (preg_match($pattern, $contfile) !== 1) {
        ...
        $classname = '\\theme_charteredcollege\\controller\\'.str_ireplace('.php', '', $contfile);
        if (class_exists($classname)) {
            $rc = new ReflectionClass($classname);
            if ($rc->isSubclassOf('\\theme_charteredcollege\\controller\\controller_abstract')) {
