...
@Component({
  selector: 'charteredcollege-feed',
  template: `
      <h2>{{ title }}</h2>
      <div id="{{ elemId }}" [@growIn]="feedItemTotal">
          <div class="charteredcollege-media-object feeditem {{feedItem.extraClasses}}" *ngFor="let feedItem of feedItems" [attr.data-from-cache]="feedItem.fromCache">
              <img *ngIf="feedItem.iconUrl !== ''" src="{{feedItem.iconUrl}}" alt="{{feedItem.iconDesc}}" [className]="feedItem.iconClass">
              <div class="charteredcollege-media-body">
                  ...
                  <span *ngIf="feedItem.description" class="charteredcollege-media-meta" [innerHTML]="feedItem.description">
                  </span>
              </div>
          </div>
      ...
      <a *ngIf="viewMoreEnabled && nextPage >= 0" href="javascript: void(0);" class="charteredcollege-personal-menu-more"
         (click)="getFeed($event)">
          <small>{{viewMoreMessage}}</small>
      </a>
      <a *ngIf="nextPage === -1 && showReload" href="javascript: void(0);" class="charteredcollege-personal-menu-more"
         (click)="purgeDataAndResetFeed()">
          <small>{{reloadMessage}}</small>
      </a>
      <span *ngIf="fetchingData" class="charteredcollege-personal-menu-more charteredcollege-personal-menu-feed-loading"></span>
  `,
  animations: [
    trigger('growIn', [
...
export class FeedComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.initialValue) {
      ...
      if (document.querySelectorAll('body.charteredcollege-pm-open').length > 0) {
        this.resetFeed();
      }
    }
  }
  ...
  @HostListener('document:charteredcollegePersonalMenuOpen', ['$event'])
